...
' ws 中 title 所在的列 其值 等于 searchValue 的 行 构成的数组
Function getRowArrWithSameValueInTitle(ws As Worksheet, titleRow As Long, title As String, dataRow As Long, searchValue As Variant) As Variant
    Dim groupCol As Long: groupCol = getColumnArrayByTitle(wsGroup, titleRow, Array(title))(0) ' 所在列
    Dim matchedRows() As Long
    Dim rowCount As Long: rowCount = 0
    Dim currentRow As Long
    Dim lastRow As Long
    lastRow = ws.Cells(ws.Rows.count, groupCol).End(xlUp).Row ' 定位最后一行
