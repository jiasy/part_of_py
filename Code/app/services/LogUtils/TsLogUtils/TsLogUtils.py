...
class TsLogUtils(BaseInService):

    def __init__(self, belongToService_):
        super().__init__(belongToService_)

    def create(self):
        super(TsLogUtils, self).create()

    def destroy(self):
        super(TsLogUtils, self).destroy()
    ...
    def initTypeScriptProject(self, nameSpace_: str):
        if nameSpace_ == "support":
            ...
            sys.exit(1)
        _tsProjectRoot = os.path.join(self.subResPath, nameSpace_)
        ...
        if not os.path.exists(_packageJsonFile):
            ...
            _supportPackageJson = os.path.join(self.subResPath, "support", "package.json")  # 模板
        ...
        if not os.path.exists(_nsTsConfigJson):
            ...
            _supportTsConfigJson = os.path.join(self.subResPath, "support", "tsconfig.json")  # 模板
    ...
    def addToolTo(self, nameSpace_: str, toolName_: str):
        _tsAnalyseToolSrc = os.path.join(self.subResPath, "support", "Tools", f"{toolName_}.ts")
        if not os.path.exists(_tsAnalyseToolSrc):
            print(f"ERROR : {toolName_} 不存在。")
            sys.exit(1)
        _tsAnalyseToolTarFolder = os.path.join(self.subResPath, nameSpace_, "src", "Tools")
    ...
    def doToolIn(self, nameSpace_: str, toolName_: str, paramList_: list[str] = None):
        ...
        _tsProjectFolder = os.path.join(self.subResPath, nameSpace_)  # 工程路径
    ...
    def getTsResult(self, nameSpace_: str, tsRelativePath_: str, paramList_: list[str] = None):
        _tsProjectPath = os.path.join(self.subResPath, nameSpace_)
...
if __name__ == '__main__':
    _subSvr_TsLogUtils: TsLogUtils = pyServiceUtils.getSubSvr(__file__)
    print('_subSvr.subResPath = ' + str(_subSvr_TsLogUtils.subResPath))
    pyServiceUtils.printSubSvrCode(__file__)
    sys.exit(1)
    # 创建工程
    _subSvr_TsLogUtils.initTypeScriptProject("AnalyseTS")
    # 在工程内添加工具并执行
    _subSvr_TsLogUtils.doToolIn(
        "AnalyseTS",
        "TsAnalyse",
        [
